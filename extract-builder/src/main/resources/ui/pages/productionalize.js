      ...
      function draw() {
        ...
        if (step === 5) {
          ...
          content.append(title('Review and confirm', canGo ? 'The dry run checks the layout, the schedule, the partner route, the contract, the BAA and the file name before anything changes.' : isLive ? 'Save updates the live settings; the next scheduled run uses them.' : 'This version is not approved yet; you can save the settings for later.'),
            h('div', { class: 'grid2' }, h('div', { class: 'card pad' }, h('div', { class: 'eyebrow mb' }, 'Summary'), kv('File', h('span', { class: 'mono' }, cfg.fileName.pattern)), kv('Schedule', cfg.schedule.preset.toLowerCase() + (cfg.schedule.preset === 'CUSTOM' ? ' ' + cfg.schedule.cron : ' at ' + cfg.schedule.time) + ' ' + cfg.schedule.timezone), kv('Holidays', (cfg.schedule.holidayPolicy || '').toLowerCase().replace(/_/g, ' ')), kv('Route', cfg.delivery.route + (partner ? ' · ' + partner.name : '')), kv('Zero rows', cfg.delivery.zeroRowPolicy.toLowerCase().replace('_', ' ')), kv('Quality gate', cfg.quality.enabled ? 'on, ±' + cfg.quality.rowCountVariancePct + '%, ' + cfg.quality.lookupMissMax + ' misses' : 'off'), kv('SLA', 'by ' + cfg.sla.deliverBy), kv('PGP', cfg.pgpBy.toLowerCase()), prev ? kv('Replaces', 'v' + prev.versionNo + ', live since ' + fmt.date(prev.productionizedAt)) : null),
